...
async def send_post(bot: Bot, tg_channel: str, text: str, photos: list, docs: list, disable_web_page_preview: bool, num_tries: int = 0) -> None:
    num_tries += 1
    if num_tries > 3:
    ...
    try:
        ...
        await send_post(bot, tg_channel, text, photos, docs, disable_web_page_preview, num_tries)
...
async def send_text_post(bot: Bot, tg_channel: str, text: str, disable_web_page_preview: bool) -> None:
    if not text:
        return

    if len(text) < 4096:
        await bot.send_message(tg_channel, text, parse_mode=types.ParseMode.HTML, disable_web_page_preview=disable_web_page_preview)
    else:
        ...
        for part in prepared_text_parts:
            await bot.send_message(tg_channel, part, parse_mode=types.ParseMode.HTML, disable_web_page_preview=disable_web_page_preview)
...
async def send_photo_post(bot: Bot, tg_channel: str, text: str, photos: list, disable_web_page_preview: bool) -> None:
    if len(text) <= 1024:
        ...
        if len(prepared_text) <= 4096:
            await bot.send_message(tg_channel, prepared_text, parse_mode=types.ParseMode.HTML, disable_web_page_preview=disable_web_page_preview)
        else:
            await send_text_post(bot, tg_channel, text, disable_web_page_preview=disable_web_page_preview)
